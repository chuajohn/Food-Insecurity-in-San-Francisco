...
mutate(NAME = census_tract) 
  ...
  write_rds(opp_zones, "App/shiny_data/opp_zones.rds")
...
bins <- c(0, 5, 10)
pal <- colorBin("Greens", domain = tract_data$Value, bins = bins)
...
income <- read_xlsx("raw_data/income.xlsx") %>%
  ...
  write_rds(income, "App/shiny_data/income.rds")
...
```{r}
CIH_data <- read_csv("yourStataFile.csv") 
